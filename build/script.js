function Y(M,C,b){for(let L=C;L<b;++L){A[L]={},A[L].tooth=[];for(let E=0;E<M;++E)A[L].tooth.push("transparent")}}function E2(M){let C=0;switch(M){case 1:case 2:case 3:case 5:C=1;break;case 4:C=2;break;case 6:C=4;break;case 7:case 8:C=3;break}return C}function B2(M){let C=0;switch(M){case 1:case 2:case 3:case 4:case 5:C=1;break;case 6:case 7:case 8:C=3;break}return C}function x(M,C){for(let b=1;b<=8;++b){A[C+b].canals=[];for(let L=0;L<M(b);++L)A[C+b].canals.push("transparent")}}function N2(M){for(let C=1;C<=3;++C)A[M+C].canals=["transparent"];for(let C=4;C<=5;++C)A[M+C].canals=["transparent","transparent","transparent","transparent"]}function q2(M){if(M<30)x(E2,M);else if(M<50)x(B2,M);else N2(M)}function p(){for(let M=1;M<=8;M+=1){const C=M*10;if(Y(4,C+1,C+4),M<5)Y(6,C+4,C+9);else Y(6,C+4,C+6);q2(C)}}function $(M){A=M,u(A)}function g(M){F=M}function v(M){u=M}var A={},u,F="transparent";function j(){const M=document.getElementsByClassName("illness-option");if(M.length!==0){for(let C of M)if(C.addEventListener("click",(b)=>{for(let L of M)L.classList.remove("active");b.target.classList.add("active"),g(b.target.value)}),console.log(C),C.classList.contains("active"))g(C.value)}}var n=`M 6.092 11.164 C 3.906 11.164 3.906 25.32 6.092 25.32 C 7.44 31.387
        ...
        6.092 11.164 Z`,f=[`M 22.185 47.82 C 23.164 47.971 27.758 29.245 25.314 29.343 C 25.898 30.074
...
22.785 Z`],R=[`M 22.224 27.229 C 21.79
        ...
        56.726 C 19.264 57.633 22.531 27.402 22.224 27.229 Z`];var h=`M 6.757 30.599 C 11.336 35.158 2.232 81.58 6.924 80.955 C 11.007 85.807
24.213 46.852 23.955 46.545 L 23.955 32.445 C 20.43 33.965 4.073 31.755 6.757
30.599 Z`,_=[`M 24.827 33.069 L 24.493 46.911 C 28.414 47.953 23.721 80.747 26.663
    ...
    C 6.697 80.856 12.254 32.435 12.638 32.435 Z`];var d=`M 11.539 2.851 C 18.9 -0.835 42.096 1.119 40.766 2.449 C
44.678 5.083 38.25 19.531 35.94 19.207 C 36.361 20.546 18.116 25.064 17.17
19.073 C 16.061 21.384 8.549 9.08 11.539 2.851 Z`,a=[`M 13.55 15.588 C 10.216 27.712 16.046 83.057 22.264 77.931 C 28.861 85.298
    ...
    43.439 76.188 33.883 28.861 33.929 28.861 Z`];function N(M){const C=document.createElementNS("http://www.w3.org/2000/svg","path");return C.style.stroke="black",C.style.fill="none",C.setAttribute("d",M),C}function q(M,C,b){const L=document.createElementNS("http://www.w3.org/2000/svg","path");return L.style.stroke="black",L.style.fill=A[M].canals[C],L.setAttribute("d",b),L.addEventListener("click",(E)=>{E.target.style.fill=F,temp_state=A,temp_state[M].canals[C]=F,$(temp_state)}),L}function y(M){const C=document.createElementNS("http://www.w3.org/2000/svg","g");for(let b of M){const L=document.createElementNS("http://www.w3.org/2000/svg","path");L.style.stroke="black",L.style.fill="none",L.setAttribute("d",b),C.appendChild(L)}return C}function M2(M,C,b,L,E){let e={},Z={},m=[];switch(C%10){case 1:e=N(d),Z=y(a),m=i.map((k,B)=>q(C,B,k));break;case 2:{e=N(I),Z=y(J),m=K.map((k,B)=>q(C,B,k));break}case 3:{e=N(O),Z=y(P),m=Q.map((k,B)=>q(C,B,k));break}case 4:e=N(H),Z=y(t),m=C2.map((k,B)=>q(C,B,k));break;case 5:e=N(H),Z=y(z),m=R.map((k,B)=>q(C,B,k));break;case 6:e=N(G),Z=y(S),m=T.map((k,B)=>q(C,B,k));break;case 7:case 8:e=N(h),Z=y(_),m=o.map((k,B)=>q(C,B,k));break}M.appendChild(V(e,Z,m,E,-1)),M.appendChild(X(C,b,L))}function Z2(M,C,b,L,E){M.appendChild(X(C,b,L));let e={},Z={},m=[];switch(C%10){case 1:case 2:{e=N(I),Z=y(J),m=K.map((k,B)=>q(C,B,k));break}case 3:{e=N(O),Z=y(P),m=Q.map((k,B)=>q(C,B,k));break}case 4:e=N(H),Z=y(r),m=R.map((k,B)=>q(C,B,k));break;case 5:e=N(H),Z=y(z),m=R.map((k,B)=>q(C,B,k));break;case 6:e=N(G),Z=y(l),m=w.map((k,B)=>q(C,B,k));break;case 7:case 8:e=N(n),Z=y(f),m=c.map((k,B)=>q(C,B,k));break}M.appendChild(V(e,Z,m,E,1))}function V(M,C,b,L,E){const e=document.createElementNS("http://www.w3.org/2000/svg","svg");e.viewBox="0 0 48 90",e.style.width="48px",e.style.height="90px";const Z=document.createElementNS("http://www.w3.org/2000/svg","g");tx=L===-1?48:0,ty=E===-1?90:0,Z.setAttribute("transform",`translate(${tx},${ty}) scale(${L}, ${E})`),Z.appendChild(M),Z.appendChild(C);for(let m of b)Z.appendChild(m);return e.appendChild(Z),e}function W(M,C,b,L,E,e){for(let Z=C;b(Z);Z+=L){const m=document.createElement("div");m.style.display="flex",m.style.flexDirection="column";const k=document.createElement("p");if(k.innerHTML=Z,Z<50)if(Z<30){if(Z>20)M2(m,Z,E,e,-1);else M2(m,Z,E,e,1);m.appendChild(k)}else if(m.appendChild(k),Z>40)Z2(m,Z,E,e,1),M.style.gridColumnStart=1,M.style.gridRowStart=2;else Z2(m,Z,E,e,-1),M.style.gridColumnStart=2,M.style.gridRowStart=2;else if(Z<70){if(Z>60)U(m,Z,E,e,-1,-1);else U(m,Z,E,e,1,-1);m.appendChild(k)}else if(m.appendChild(k),Z>80)U(m,Z,E,e,1,1),M.style.gridColumnStart=1,M.style.gridRowStart=2;else U(m,Z,E,e,-1,1),M.style.gridColumnStart=2,M.style.gridRowStart=2;M.appendChild(m)}}function U(M,C,b,L,E,e){let Z={},m={},k=[];switch(C%10){case 1:case 2:{Z=N(I),m=y(J),k=K.map((B,D)=>q(C,D,B));break}case 3:{Z=N(O),m=y(P),k=Q.map((B,D)=>q(C,D,B));break}case 4:Z=N(G),m=y(S),k=T.map((B,D)=>q(C,D,B));break;case 5:Z=N(G),m=y(S),k=T.map((B,D)=>q(C,D,B));break}if(e===1)M.appendChild(X(C,b,L)),M.appendChild(V(Z,m,k,E,e));else M.appendChild(V(Z,m,k,E,e)),M.appendChild(X(C,b,L))}function m2(){return b2(5,8,5)}function k2(){return b2(1,4,8)}function y2(M,C,b){W(M,C+1,(L)=>{return L<=C+3},1,e2,4),W(M,C+4,(L)=>{return L<=C+b},1,L2,6)}function A2(M,C,b){W(M,C+b,(L)=>{return L>=C+4},-1,L2,6),W(M,C+3,(L)=>{return L>C},-1,e2,4)}function b2(M,C,b){const L=document.createElement("div");L.style.display="grid",L.style.gridTemplateColumns="repeat(2, 1fr)",L.style.gridTemplateRows="repeat(2, 1fr)";for(let E=M;E<=C;E+=1){const e=document.createElement("div");e.style.display="flex",e.style.flexDirection="row";const Z=E*10;switch(E){case 1:case 4:case 5:case 8:A2(e,Z,b);break;case 2:case 3:case 6:case 7:y2(e,Z,b);break}L.appendChild(e)}return L}function X(M,C,b){const L=document.createElement("div");for(let E=0;E<b;++E){const e=C[E],Z=e[1];L.appendChild(D2(M,E,Z[0],Z[1],A[M].tooth[E],e[0]))}return L.style.position="relative",L.style.width="48px",L.style.height="48px",L}function D2(M,C,b,L,E,e){const Z=document.createElementNS("http://www.w3.org/2000/svg","svg");switch(Z.addEventListener("click",(m)=>{s=m.target.parentElement,s.style.fill=F;const k=A;k[M].tooth[C]=F,$(k)}),Z.style.viewBox=`0 0 ${b} ${L}`,Z.style.width=b,Z.style.height=L,Z.style.position="absolute",Z.style.fill=E,Z.style.stroke="black",Z.innerHTML=e,C){case 0:case 3:break;case 1:Z.style.right=0;break;case 2:Z.style.bottom=0;break;case 4:Z.style.top="12px",Z.style.left="12px";break;case 5:Z.style.top="12px",Z.style.left="24px";break}return Z}var L2=[['<path d=" M 0 0 h 48 l -12 12 h -24 z "></path>',[48,12]],['<path d=" M 12 0 v 48 l -12 -12 v -24 z "></path>',[12,48]],['<path d=" M 0 12 h 48 l -12 -12 h -24 z " ></path>',[48,12]],['<path d=" M 0 0 v 48 l 12 -12 v -24 z " ></path>',[12,48]],['<path d=" M 0 0 h 12 v 24 h -24 z " ></path>',[12,24]],['<path d=" M 0 0 h 12 v 24 h -12 z " ></path>',[12,24]]],e2=[['<path d=" M 0 0 h 48 l -12 24 h -24 z " ></path> ',[48,24]],['<path d=" M 12 0 v 48 l -12 -24 z " ></path>',[12,48]],['<path d=" M 0 24 h 48 l -12 -24 h -24 z"></path>',[48,24]],['<path d=" M 0 0 v 48 l 12 -24 z " ></path>',[12,48]]];self.onload=function(){console.log("we ball"),v(console.log),p(),j();const M=k2(),C=document.createElement("div");C.style.width="50%",C.appendChild(M),document.body.appendChild(C);const b=m2(),L=document.createElement("div");L.style.width="50%",L.appendChild(b),document.body.appendChild(L)};
