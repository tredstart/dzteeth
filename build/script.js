function X(M,C,m){for(let L=C;L<m;++L){y[L]={},y[L].tooth=[];for(let g=0;g<M;++g)y[L].tooth.push("transparent")}}function g2(M){let C=0;switch(M){case 1:case 2:case 3:case 5:C=1;break;case 4:C=2;break;case 6:C=4;break;case 7:case 8:C=3;break}return C}function E2(M){let C=0;switch(M){case 1:case 2:case 3:case 4:case 5:C=1;break;case 6:case 7:case 8:C=3;break}return C}function n(M,C){for(let m=1;m<=8;++m){y[C+m].canals=[];for(let L=0;L<M(m);++L)y[C+m].canals.push("transparent")}}function B2(M){for(let C=1;C<=3;++C)y[M+C].canals=["transparent"];for(let C=4;C<=5;++C)y[M+C].canals=["transparent","transparent","transparent","transparent"]}function N2(M){if(M<30)n(g2,M);else if(M<50)n(E2,M);else B2(M)}function u(){for(let M=1;M<=8;M+=1){const C=M*10;if(X(4,C+1,C+4),M<5)X(6,C+4,C+9);else X(6,C+4,C+6);N2(C)}}function Y(M,C,m,L){y[M][C][m]=L,p(M,C,m,L)}function $(M){A.part_color=M,A.canal_color=null}function x(M){p=M}var y={},p,A={part_color:null,canal_color:null};function j(){const M=document.getElementsByClassName("illness-option");if(M.length!==0){for(let C of M)if(C.addEventListener("click",(m)=>{for(let L of M)L.classList.remove("active");m.target.classList.add("active"),$(m.target.value)}),console.log(C),C.classList.contains("active"))$(C.value)}}var v=`M 6.092 11.164 C 3.906 11.164 3.906 25.32 6.092 25.32 C 7.44 31.387
...
26.059 29.343 C 26.26 30.501 23.453 48.15 22.185 47.82 Z`],c=[`M 16.97 29.343 L 21.142 29.641 C 22.715 30.54 11.015 65.491 9.817
        ...
        29.492 Z`];var F=`M 6.092 12.654 C 3.906
...
22.785 Z`],Q=[`M 22.224 27.229 C 21.79
        28.572 25.428 28.167 26.063 27.431 C 23.591 48.585 18.658 58.317 19.194
        56.726 C 19.264 57.633 22.531 27.402 22.224 27.229 Z`];var o=`M 6.757 30.599 C 11.336 35.158 2.232 81.58 6.924 80.955 C 11.007 85.807
...
30.599 Z`,_=[`M 24.827 33.069 L 24.493 46.911 C 28.414 47.953 23.721 80.747 26.663
    ...
    14.502 Z`],h=[`M 30.504 32.101 L 33.342 31.767 C 34.25 31.767 28.557 80.355 27.164 80.355
...
19.073 C 16.061 21.384 8.549 9.08 11.539 2.851 Z`,d=[`M 13.55 15.588 C 10.216 27.712 16.046 83.057 22.264 77.931 C 28.861 85.298
    ...
    C 27.723 72.79 26.369 47.051 25.348 47.362 Z`];var R=[`M 7.248 26.85 C 12.431 27.989 3.338 83.026 6.712 83.026 C 11.558 88.195
    ...
    39.644 24.276 40.391 Z`],S=[`M 13.684 28.995 L 17.169 29.129 C 17.34 29.163 9.881 82.909 8.455 82.624 C
    ...
    43.439 76.188 33.883 28.861 33.929 28.861 Z`];function B(M){const C=document.createElementNS("http://www.w3.org/2000/svg","path");return C.style.stroke="black",C.style.fill="none",C.setAttribute("d",M),C}function N(M,C,m){const L=document.createElementNS("http://www.w3.org/2000/svg","path");return L.style.stroke="black",L.style.fill=y[M].canals[C],L.setAttribute("d",m),L.addEventListener("click",(g)=>{if(A.canal_color!==null)g.target.style.fill=A.part_color,Y(M,"canals",C,temp_state,A.part_color)}),L}function q(M){const C=document.createElementNS("http://www.w3.org/2000/svg","g");for(let m of M){const L=document.createElementNS("http://www.w3.org/2000/svg","path");L.style.stroke="black",L.style.fill="none",L.setAttribute("d",m),C.appendChild(L)}return C}function M2(M,C,m,L,g){let e={},Z={},k=[];switch(C%10){case 1:e=B(a),Z=q(d),k=i.map((b,E)=>N(C,E,b));break;case 2:{e=B(H),Z=q(I),k=J.map((b,E)=>N(C,E,b));break}case 3:{e=B(K),Z=q(O),k=P.map((b,E)=>N(C,E,b));break}case 4:e=B(G),Z=q(t),k=C2.map((b,E)=>N(C,E,b));break;case 5:e=B(G),Z=q(z),k=Q.map((b,E)=>N(C,E,b));break;case 6:e=B(F),Z=q(R),k=S.map((b,E)=>N(C,E,b));break;case 7:case 8:e=B(o),Z=q(_),k=h.map((b,E)=>N(C,E,b));break}M.appendChild(U(e,Z,k,g,-1)),M.appendChild(W(C,m,L))}function Z2(M,C,m,L,g){M.appendChild(W(C,m,L));let e={},Z={},k=[];switch(C%10){case 1:case 2:{e=B(H),Z=q(I),k=J.map((b,E)=>N(C,E,b));break}case 3:{e=B(K),Z=q(O),k=P.map((b,E)=>N(C,E,b));break}case 4:e=B(G),Z=q(r),k=Q.map((b,E)=>N(C,E,b));break;case 5:e=B(G),Z=q(z),k=Q.map((b,E)=>N(C,E,b));break;case 6:e=B(F),Z=q(l),k=w.map((b,E)=>N(C,E,b));break;case 7:case 8:e=B(v),Z=q(f),k=c.map((b,E)=>N(C,E,b));break}M.appendChild(U(e,Z,k,g,1))}function U(M,C,m,L,g){const e=document.createElementNS("http://www.w3.org/2000/svg","svg");e.viewBox="0 0 48 90",e.style.width="48px",e.style.height="90px";const Z=document.createElementNS("http://www.w3.org/2000/svg","g");tx=L===-1?48:0,ty=g===-1?90:0,Z.setAttribute("transform",`translate(${tx},${ty}) scale(${L}, ${g})`),Z.appendChild(M),Z.appendChild(C);for(let k of m)Z.appendChild(k);return e.appendChild(Z),e}function V(M,C,m,L,g,e){for(let Z=C;m(Z);Z+=L){const k=document.createElement("div");k.style.display="flex",k.style.flexDirection="column";const b=document.createElement("p");if(b.innerHTML=Z,Z<50)if(Z<30){if(Z>20)M2(k,Z,g,e,-1);else M2(k,Z,g,e,1);k.appendChild(b)}else if(k.appendChild(b),Z>40)Z2(k,Z,g,e,1),M.style.gridColumnStart=1,M.style.gridRowStart=2;else Z2(k,Z,g,e,-1),M.style.gridColumnStart=2,M.style.gridRowStart=2;else if(Z<70){if(Z>60)T(k,Z,g,e,-1,-1);else T(k,Z,g,e,1,-1);k.appendChild(b)}else if(k.appendChild(b),Z>80)T(k,Z,g,e,1,1),M.style.gridColumnStart=1,M.style.gridRowStart=2;else T(k,Z,g,e,-1,1),M.style.gridColumnStart=2,M.style.gridRowStart=2;M.appendChild(k)}}function T(M,C,m,L,g,e){let Z={},k={},b=[];switch(C%10){case 1:case 2:{Z=B(H),k=q(I),b=J.map((E,D)=>N(C,D,E));break}case 3:{Z=B(K),k=q(O),b=P.map((E,D)=>N(C,D,E));break}case 4:Z=B(F),k=q(R),b=S.map((E,D)=>N(C,D,E));break;case 5:Z=B(F),k=q(R),b=S.map((E,D)=>N(C,D,E));break}if(e===1)M.appendChild(W(C,m,L)),M.appendChild(U(Z,k,b,g,e));else M.appendChild(U(Z,k,b,g,e)),M.appendChild(W(C,m,L))}function m2(){return b2(5,8,5)}function k2(){return b2(1,4,8)}function q2(M,C,m){V(M,C+1,(L)=>{return L<=C+3},1,e2,4),V(M,C+4,(L)=>{return L<=C+m},1,L2,6)}function y2(M,C,m){V(M,C+m,(L)=>{return L>=C+4},-1,L2,6),V(M,C+3,(L)=>{return L>C},-1,e2,4)}function b2(M,C,m){const L=document.createElement("div");L.style.display="grid",L.style.gridTemplateColumns="repeat(2, 1fr)",L.style.gridTemplateRows="repeat(2, 1fr)";for(let g=M;g<=C;g+=1){const e=document.createElement("div");e.style.display="flex",e.style.flexDirection="row";const Z=g*10;switch(g){case 1:case 4:case 5:case 8:y2(e,Z,m);break;case 2:case 3:case 6:case 7:q2(e,Z,m);break}L.appendChild(e)}return L}function W(M,C,m){const L=document.createElement("div");for(let g=0;g<m;++g){const e=C[g],Z=e[1];L.appendChild(A2(M,g,Z[0],Z[1],y[M].tooth[g],e[0]))}return L.style.position="relative",L.style.width="48px",L.style.height="48px",L}function A2(M,C,m,L,g,e){const Z=document.createElementNS("http://www.w3.org/2000/svg","svg");switch(Z.addEventListener("click",(k)=>{if(A.part_color!==null)s=k.target.parentElement,s.style.fill=A.part_color,Y(M,"tooth",C,A.part_color)}),Z.style.viewBox=`0 0 ${m} ${L}`,Z.style.width=m,Z.style.height=L,Z.style.position="absolute",Z.style.fill=g,Z.style.stroke="black",Z.innerHTML=e,C){case 0:case 3:break;case 1:Z.style.right=0;break;case 2:Z.style.bottom=0;break;case 4:Z.style.top="12px",Z.style.left="12px";break;case 5:Z.style.top="12px",Z.style.left="24px";break}return Z}var L2=[['<path d=" M 0 0 h 48 l -12 12 h -24 z "></path>',[48,12]],['<path d=" M 12 0 v 48 l -12 -12 v -24 z "></path>',[12,48]],['<path d=" M 0 12 h 48 l -12 -12 h -24 z " ></path>',[48,12]],['<path d=" M 0 0 v 48 l 12 -12 v -24 z " ></path>',[12,48]],['<path d=" M 0 0 h 12 v 24 h -24 z " ></path>',[12,24]],['<path d=" M 0 0 h 12 v 24 h -12 z " ></path>',[12,24]]],e2=[['<path d=" M 0 0 h 48 l -12 24 h -24 z " ></path> ',[48,24]],['<path d=" M 12 0 v 48 l -12 -24 z " ></path>',[12,48]],['<path d=" M 0 24 h 48 l -12 -24 h -24 z"></path>',[48,24]],['<path d=" M 0 0 v 48 l 12 -24 z " ></path>',[12,48]]];function D2(M,C,m,L){console.log(M,C,m,L),console.log(y)}self.onload=function(){console.log("we ball"),x(D2),u(),j();const M=k2(),C=document.createElement("div");C.style.width="50%",C.appendChild(M),document.body.appendChild(C);const m=m2(),L=document.createElement("div");L.style.width="50%",L.appendChild(m),document.body.appendChild(L)};
