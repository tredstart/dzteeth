function Y(M,C,m){for(let L=C;L<m;++L){A[L]={},A[L].tooth=[];for(let B=0;B<M;++B)A[L].tooth.push("transparent")}}function g2(M){let C=0;switch(M){case 1:case 2:case 3:case 5:C=1;break;case 4:C=2;break;case 6:C=4;break;case 7:case 8:C=3;break}return C}function N2(M){let C=0;switch(M){case 1:case 2:case 3:case 4:case 5:C=1;break;case 6:case 7:case 8:C=3;break}return C}function y(M,C){for(let m=1;m<=8;++m){A[C+m].canals=[];for(let L=0;L<M(m);++L)A[C+m].canals.push("transparent")}}function q2(M){for(let C=1;C<=3;++C)A[M+C].canals=["transparent"];for(let C=4;C<=5;++C)A[M+C].canals=["transparent","transparent","transparent","transparent"]}function A2(M){if(M<30)y(g2,M);else if(M<50)y(N2,M);else q2(M)}function u(M=null){if(M===null)for(let C=1;C<=8;C+=1){const m=C*10;if(Y(4,m+1,m+4),C<5)Y(6,m+4,m+9);else Y(6,m+4,m+6);A2(m)}else A=M}function $(M,C,m,L){A[M][C][m]=L,x(M,C,m,L)}function j(M){D.part_color=M,D.canal_color=null}function p(M){D.canal_color=M,D.part_color=null}function v(M){x=M}var A={},x,D={part_color:null,canal_color:null};var f=`M 6.092 11.164 C 3.906 11.164 3.906 25.32 6.092 25.32 C 7.44 31.387
        ...
        6.092 11.164 Z`,n=[`M 22.185 47.82 C 23.164 47.971 27.758 29.245 25.314 29.343 C 25.898 30.074
...
19.073 C 16.061 21.384 8.549 9.08 11.539 2.851 Z`,d=[`M 13.55 15.588 C 10.216 27.712 16.046 83.057 22.264 77.931 C 28.861 85.298
    ...
    43.439 76.188 33.883 28.861 33.929 28.861 Z`];function g(M){const C=document.createElementNS("http://www.w3.org/2000/svg","path");return C.style.stroke="black",C.style.fill="none",C.setAttribute("d",M),C}function N(M,C,m){const L=document.createElementNS("http://www.w3.org/2000/svg","path");return L.style.stroke="black",L.style.fill=A[M].canals[C],L.setAttribute("d",m),L.addEventListener("click",(B)=>{if(D.canal_color!==null)B.target.style.fill=D.canal_color,$(M,"canals",C,D.canal_color)}),L}function q(M){const C=document.createElementNS("http://www.w3.org/2000/svg","g");for(let m of M){const L=document.createElementNS("http://www.w3.org/2000/svg","path");L.style.stroke="black",L.style.fill="none",L.setAttribute("d",m),C.appendChild(L)}return C}function M2(M,C,m,L,B){let k={},Z={},e=[];switch(C%10){case 1:k=g(a),Z=q(d),e=i.map((b,E)=>N(C,E,b));break;case 2:{k=g(I),Z=q(J),e=K.map((b,E)=>N(C,E,b));break}case 3:{k=g(O),Z=q(P),e=Q.map((b,E)=>N(C,E,b));break}case 4:k=g(H),Z=q(t),e=C2.map((b,E)=>N(C,E,b));break;case 5:k=g(H),Z=q(z),e=R.map((b,E)=>N(C,E,b));break;case 6:k=g(G),Z=q(S),e=T.map((b,E)=>N(C,E,b));break;case 7:case 8:k=g(_),Z=q(h),e=o.map((b,E)=>N(C,E,b));break}M.appendChild(V(k,Z,e,B,-1)),M.appendChild(X(C,m,L))}function Z2(M,C,m,L,B){M.appendChild(X(C,m,L));let k={},Z={},e=[];switch(C%10){case 1:case 2:{k=g(I),Z=q(J),e=K.map((b,E)=>N(C,E,b));break}case 3:{k=g(O),Z=q(P),e=Q.map((b,E)=>N(C,E,b));break}case 4:k=g(H),Z=q(r),e=R.map((b,E)=>N(C,E,b));break;case 5:k=g(H),Z=q(z),e=R.map((b,E)=>N(C,E,b));break;case 6:k=g(G),Z=q(l),e=w.map((b,E)=>N(C,E,b));break;case 7:case 8:k=g(f),Z=q(n),e=c.map((b,E)=>N(C,E,b));break}M.appendChild(V(k,Z,e,B,1))}function V(M,C,m,L,B){const k=document.createElementNS("http://www.w3.org/2000/svg","svg");k.viewBox="0 0 48 90",k.style.width="48px",k.style.height="90px";const Z=document.createElementNS("http://www.w3.org/2000/svg","g");tx=L===-1?48:0,ty=B===-1?90:0,Z.setAttribute("transform",`translate(${tx},${ty}) scale(${L}, ${B})`),Z.appendChild(M),Z.appendChild(C);for(let e of m)Z.appendChild(e);return k.appendChild(Z),k}function W(M,C,m,L,B,k){for(let Z=C;m(Z);Z+=L){const e=document.createElement("div");e.style.display="flex",e.style.flexDirection="column";const b=document.createElement("p");if(b.innerHTML=Z,Z<50)if(Z<30){if(Z>20)M2(e,Z,B,k,-1);else M2(e,Z,B,k,1);e.appendChild(b)}else if(e.appendChild(b),Z>40)Z2(e,Z,B,k,1),M.style.gridColumnStart=1,M.style.gridRowStart=2;else Z2(e,Z,B,k,-1),M.style.gridColumnStart=2,M.style.gridRowStart=2;else if(Z<70){if(Z>60)U(e,Z,B,k,-1,-1);else U(e,Z,B,k,1,-1);e.appendChild(b)}else if(e.appendChild(b),Z>80)U(e,Z,B,k,1,1),M.style.gridColumnStart=1,M.style.gridRowStart=2;else U(e,Z,B,k,-1,1),M.style.gridColumnStart=2,M.style.gridRowStart=2;M.appendChild(e)}}function U(M,C,m,L,B,k){let Z={},e={},b=[];switch(C%10){case 1:case 2:{Z=g(I),e=q(J),b=K.map((E,F)=>N(C,F,E));break}case 3:{Z=g(O),e=q(P),b=Q.map((E,F)=>N(C,F,E));break}case 4:Z=g(G),e=q(S),b=T.map((E,F)=>N(C,F,E));break;case 5:Z=g(G),e=q(S),b=T.map((E,F)=>N(C,F,E));break}if(k===1)M.appendChild(X(C,m,L)),M.appendChild(V(Z,e,b,B,k));else M.appendChild(V(Z,e,b,B,k)),M.appendChild(X(C,m,L))}function k2(){return B2(5,8,5)}function e2(){return B2(1,4,8)}function D2(M,C,m){W(M,C+1,(L)=>{return L<=C+3},1,m2,4),W(M,C+4,(L)=>{return L<=C+m},1,L2,6)}function F2(M,C,m){W(M,C+m,(L)=>{return L>=C+4},-1,L2,6),W(M,C+3,(L)=>{return L>C},-1,m2,4)}function B2(M,C,m){const L=document.createElement("div");L.style.display="grid",L.style.gridTemplateColumns="repeat(2, 1fr)",L.style.gridTemplateRows="repeat(2, 1fr)";for(let B=M;B<=C;B+=1){const k=document.createElement("div");k.style.display="flex",k.style.flexDirection="row";const Z=B*10;switch(B){case 1:case 4:case 5:case 8:F2(k,Z,m);break;case 2:case 3:case 6:case 7:D2(k,Z,m);break}L.appendChild(k)}return L}function X(M,C,m){const L=document.createElement("div");for(let B=0;B<m;++B){const k=C[B],Z=k[1];L.appendChild(G2(M,B,Z[0],Z[1],A[M].tooth[B],k[0]))}return L.style.position="relative",L.style.width="48px",L.style.height="48px",L}function G2(M,C,m,L,B,k){const Z=document.createElementNS("http://www.w3.org/2000/svg","svg");switch(Z.addEventListener("click",(e)=>{if(D.part_color!==null)s=e.target.parentElement,s.style.fill=D.part_color,$(M,"tooth",C,D.part_color)}),Z.style.viewBox=`0 0 ${m} ${L}`,Z.style.width=m,Z.style.height=L,Z.style.position="absolute",Z.style.fill=B,Z.style.stroke="black",Z.innerHTML=k,C){case 0:case 3:break;case 1:Z.style.right=0;break;case 2:Z.style.bottom=0;break;case 4:Z.style.top="12px",Z.style.left="12px";break;case 5:Z.style.top="12px",Z.style.left="24px";break}return Z}var L2=[['<path d=" M 0 0 h 48 l -12 12 h -24 z "></path>',[48,12]],['<path d=" M 12 0 v 48 l -12 -12 v -24 z "></path>',[12,48]],['<path d=" M 0 12 h 48 l -12 -12 h -24 z " ></path>',[48,12]],['<path d=" M 0 0 v 48 l 12 -12 v -24 z " ></path>',[12,48]],['<path d=" M 0 0 h 12 v 24 h -24 z " ></path>',[12,24]],['<path d=" M 0 0 h 12 v 24 h -12 z " ></path>',[12,24]]],m2=[['<path d=" M 0 0 h 48 l -12 24 h -24 z " ></path> ',[48,24]],['<path d=" M 12 0 v 48 l -12 -24 z " ></path>',[12,48]],['<path d=" M 0 24 h 48 l -12 -24 h -24 z"></path>',[48,24]],['<path d=" M 0 0 v 48 l 12 -24 z " ></path>',[12,48]]];function b2(M,C){for(let m of M)if(m.addEventListener("click",(L)=>{for(let B of M)B.classList.remove("active");L.target.classList.add("active"),C(L.target.value)}),m.classList.contains("active"))C(m.value)}function E2(M,C){const m=document.getElementsByClassName(M),L=document.getElementsByClassName(C);if(m.length!==0)b2(m,j);if(L.length!==0)b2(L,p)}self.dzteeth={configure_options:E2,global_state:A,init_global_state:u,render_adult_teeth:e2,render_child_teeth:k2,set_on_state_change:v};
